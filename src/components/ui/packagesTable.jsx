...
function PackagesTable() {
  ...
  const sortColumns = [
    ...
    { value: "package_start_date", label: "Start Date" },
    { value: "package_end_date", label: "End Date" },
    { value: "url", label: "URL" },
  ];
  ...
  const openEditDialog = (pkg) => {
    ...
    setFormData({
      ...
      package_start_date: pkg.package_start_date,
      package_end_date: pkg.package_end_date,
      url: pkg.url || "",
    });
    setFormErrors({});
  ...
  const handleEditPackage = async () => {
    ...
    try {
      ...
      if (formData.package_start_date !== editingPackage.package_start_date) {
        changedFields["Package Start Date"] = formData.package_start_date;
      }
      if (formData.package_end_date !== editingPackage.package_end_date) {
        changedFields["Package End Date"] = formData.package_end_date;
      }
      ...
      <div className="flex justify-between items-end">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            View all event packages, including dates and direct links
          </p>
        </div>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((pkg) => (
                <TableRow key={pkg.package_id}>
                  ...
                  <TableCell>{pkg.package_start_date}</TableCell>
                  <TableCell>{pkg.package_end_date}</TableCell>
                  <TableCell>
                    {pkg.url ? (
                      <a
                        href={pkg.url}
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-6 py-4 relative">
            ...
            >
              ...
              {/* Start/End Date (Date Range Picker) */}
              <div className="space-y-2">
                <Label>Package Dates</Label>
                <DatePickerWithRange
                  date={{
                    from: formData.package_start_date
                      ? new Date(formData.package_start_date)
                      : undefined,
                    to: formData.package_end_date
                      ? new Date(formData.package_end_date)
                      : undefined,
                  }}
                  setDate={({ from, to }) => {
                    handleFieldChange(
